...
- **A waived release always failed its own Publish run.** The Bridge E2E attestation can be waived, but the only channel was the `waive_bridge_e2e` dispatch input, and `release: published` accepts no inputs. So shipping a waived release meant dispatching Publish manually and then watching the release event fire a second Publish run that could not waive and failed by construction — minutes after the package was already on npm. Releases 3.2.0, 3.2.1 and 4.0.0 all shipped exactly that way, which made a red Publish run the expected outcome of a successful release and trained everyone to ignore the gate. The release trigger now has its own explicit waiver channel: `[bridge-e2e: waived]` in the GitHub release notes, matched case- and space-tolerantly, brackets required so prose about the gate cannot waive it. Waivers print `release-attestation WAIVED` naming the channel used and write a ⚠️ block to the run summary. Unchanged: CI and preship attestations remain unwaivable, and there is still no ambient or default waiver.

- **A waiver could suppress a *failed* Bridge run, not just an absent one.** The gate is documented as "a waiver covers absent evidence, never failed evidence", but the check consulted the waiver first and returned before ever fetching the commit status — so a red `mailpouch/proton-bridge-e2e` result published exactly as easily as an unrun one. The waiver channel added above would have inherited the same hole. The status is now classified before the waiver is consulted: `failure`/`error` hard-fails regardless of any waiver, `success` passes on real evidence, and only genuinely absent or still-`pending` evidence can be waived. The waiver message now also states which it was.

- **The live-Bridge send test measured Proton's latency and reported it as a mailpouch failure.** `sending.e2e` allows a self-addressed message to leave via Proton's SMTP, be processed server-side, and return over IMAP — a fully remote round-trip — but waited only 30s for it, with `testTimeout` also at 30s so the wait could not have been extended anyway. Across six identical live runs of the same commit it failed twice on that one assertion, always with `send_email` reporting `success: true`; the file passed in isolation with far more headroom. Because this is the suite behind `mailpouch/proton-bridge-e2e`, a flake there blocks releases. The bridge lane now gets a `BRIDGE_DELIVERY_WAIT_MS` (120s) budget with `testTimeout` raised above it; polling still exits the moment the message lands, so a healthy round-trip is no slower. Greenmail is in-process and keeps its 30s budget — 80/80 unchanged.
...
- **mailpouch no longer adopts another process's settings URL.** This is the headline change, and it replaces a mechanism rather than patching one.
  ...
  Previously, if the configured settings port was already serving, mailpouch probed it and — if the occupant looked like a mailpouch settings UI — adopted its URL as the address advertised through the tray, through `openBrowser`, and to agents. That address is where the user is asked to type their Bridge password, so the probe was effectively deciding who receives credential entry.

  Authenticating a loopback neighbour well enough to justify that trust took four consecutive rounds of security fixes:

  1. The probe accepted any listener echoing a boolean `hasConfig` field — a trivially forgeable shape, so whoever bound the port first won.
  2. The replacement published a nonce beside the `0o600` config but **echoed it on `/api/status`**, which is unauthenticated in loopback mode (the access token is generated only for LAN mode). Any local process could fetch the secret, wait for an unclean shutdown to leave the nonce file behind, then bind the port and replay it.
  3. The probe used Node's default HTTP agent, which is keep-alive since Node 19, leaving a **pooled socket open to listeners it had just decided not to trust**.
  4. The challenge-response proof was unbound to the responder, so a squatter on the configured port could **relay the challenge to the real instance on its fallback port** and forward the answer back.

  A fifth issue remained after all of that: the probe verified **once at startup** and then trusted the URL for the process lifetime, so a squatter taking the port later was never re-checked.

  The reuse path existed solely to suppress a warning line in the "standalone `mailpouch-settings` daemon plus stdio MCP" setup. That benefit does not justify the mechanism, so the mechanism is gone: mailpouch always binds its own port and only ever advertises a URL it serves. A port-occupant check survives to choose a log level and nothing else — a forgeable shape check is correct there, because a wrong answer costs a warning rather than a password. Net −439 lines, and the entire class of "who am I deferring credential entry to?" bugs goes with it.

  **User-visible:** running a standalone settings daemon alongside the MCP process now yields two settings UIs on different ports instead of one, and the second is logged at info rather than warned about. Mixed-version installs already behaved this way.

- **The npm-audit release gate now fails closed when the audit cannot run.** `npm audit --json` emits *valid* JSON on a registry or network failure, but with no `vulnerabilities` key — just `{ message, error }`. `check-npm-audit.mjs` read that through `report.vulnerabilities ?? {}`, turning "the audit could not run" into "the audit found nothing": the gate printed `npm-audit OK: 0 prod-dep findings` and exited 0. Since it is wired to `prepublishOnly` and the pre-push hook, a transient registry outage was enough to publish with the audit gate silently satisfied. It now detects the error shape and exits 1. Verified against an unreachable registry, before and after.

- **Lowering your permission preset during a destructive-confirm dialog now takes effect for gated callers.** The global preset is half the authorization ceiling, and it was captured once at the top of the tool-call handler — before the confirmation elicitation, which awaits user input for an unbounded time. The code already re-read authorization state after that await (its comment says "so an external revoke/change wins before dispatch"), but re-read only the grant store; the preset lives in a different file and stayed pinned to its pre-await value. So a user who grew suspicious mid-dialog and dropped their preset to `read_only` would still have the destructive call authorized under the revoked, more permissive ceiling. The preset is now re-read alongside the grant.

  Scope: the post-await re-validation block only runs for an identified caller. Where local stdio gating is disabled there is no caller to gate, so no post-await authorization re-check of any kind runs and a mid-dialog downgrade still will not take effect. That is pre-existing behaviour of the gating architecture, neither introduced nor worsened here, but the guarantee above should not be read as unconditional.

- **Remote server error text can no longer carry terminal escape sequences.** A failing SMTP/IMAP AUTH response is echoed back to the user, and `shortError()` took it verbatim apart from trimming to the first line — so ESC bytes survived into `process.stdout.write` via the TUI's connectivity view. An on-path attacker answering a cleartext `535` (SMTP requires TLS only for localhost) could clear and repaint the terminal, e.g. forging a green "connected" line over a real failure. The text is now scrubbed with `sanitizeText` at the boundary where it arrives, so both the browser UI and the TUI benefit, and the TUI additionally routes it through the existing `stripAnsi` helper — which was already applied to escalation reasons twenty lines away, but not to this field.

- **Writing the Claude Desktop config no longer widens its permissions.** `POST /api/write-claude-desktop` wrote its temp file at the umask default and then `renameSync`'d it over `claude_desktop_config.json`. Since rename replaces the inode, a user's hardened `0600` config became `0644` — and that file routinely holds API keys for *other* MCP servers, so mailpouch was exposing a third party's secrets to every local account. The temp file is now written `0600`, matching the `write-claude-code` sibling and the rest of the project's file hygiene.

- **The settings URL advertises the literal loopback address instead of `localhost`.** The server binds `127.0.0.1`, but the advertised URL said `localhost`, which on a dual-stack machine can resolve to `::1` first — so a process bound to `[::1]:<port>` could receive the browser open without ever being probed. This affected the self-bound path as much as the reuse path. The loopback settings server is always plain HTTP (TLS exists only in LAN mode) and the origin allowlist already accepted both spellings, so nothing else changes.

- **FTS index WAL sidecars are no longer world-readable.** `openFtsIndex()` called `chmodFtsFiles()` immediately after opening the database, but the `-wal` and `-shm` sidecars do not exist at that point — they are created when the `FtsIndexService` constructor enables `journal_mode = WAL`. Since `chmodFtsFiles` skips files that do not exist, the sidecars kept the umask default (0644 observed) permanently, while the main `.db` was correctly tightened to 0600. The `-wal` file holds live page data, so decrypted mail content in the search index was readable by any local user. The chmod now runs again after the service is constructed.
