...
- **The settings URL now advertises the literal loopback address instead of `localhost`.** The probe authenticates `127.0.0.1:<port>` but the URL handed to the tray, to `openBrowser`, and to agents said `localhost`, which on a dual-stack machine can resolve to `::1` first. A process bound to `[::1]:<port>` was therefore never probed, never needed the nonce, and could still receive the browser open — bypassing the identity check entirely. This affected the self-bound path too, since that also binds `127.0.0.1` and advertised `localhost`. The loopback settings server is always plain HTTP (TLS exists only in LAN mode) and the origin allowlist already accepted both spellings, so nothing else changes.
- **The settings-UI identity proof is now bound to the port serving it.** The challenge-response below proved only "someone, somewhere, holds the nonce" — every instance answered a given challenge identically. Because `/api/status` is unauthenticated in loopback mode, a process squatting the configured port could relay the probe's challenge to the *real* instance on its fallback port, forward the reply, and be believed. The proof now covers the nonce, the challenge, and the responder's own port, with length-prefixed fields so a crafted challenge cannot shift the boundary between them; a relayed proof answers for the wrong port and no longer verifies.
- **The settings-UI instance nonce is no longer sent over the wire.** The first version of the probe fix (below) echoed the nonce on `GET /api/status`, and that route is unauthenticated in loopback mode — the access token is only generated for LAN mode. Any local process could therefore fetch the nonce, wait for the real UI to die *without* a clean shutdown (which leaves the nonce file behind), then bind the port and replay it. That was a complete bypass of the control, and it defeated the exact different-local-user threat the nonce was introduced to close. The probe now sends a fresh random challenge and the server answers `sha256(nonce:challenge)`; the nonce itself never leaves the process. A proof is worthless to anyone who does not already hold the nonce, and worthless again on the next probe. `/api/status` no longer returns an `instanceId` field at all.
