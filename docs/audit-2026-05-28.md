...
> Archived evidence ledger. For routine lookup use the compact
...
> **Resolved in v3.0.52 — IMAP service hardening.** `searchSingleFolder` now runs the header value through `sanitizeImapStr` and validates the field against `/^[A-Za-z][A-Za-z0-9-]*$/`, rejecting a non-conforming field before any IMAP round-trip.
...
> **Resolved in v3.0.52 — IMAP service hardening.** Both IDLE handlers now call a single `evictInboxCacheEntries()` helper that snapshots `Array.from(emailCache.keys())` before deleting and matches the folder case-insensitively (`folder.toLowerCase() === 'inbox'`), so aliased INBOX paths are invalidated and concurrent `setCacheEntry` can't corrupt the iteration.
...
> **Resolved in v3.0.52 — IMAP service hardening.** New exported `normalizeAddressList(field)` accepts the `AddressObject | AddressObject[] | undefined` shape and flattens it to a string array; applied at all three `to`/`cc` extraction sites (`getEmailById`, the list fetch, and the attachment full-source re-fetch).
...
> **Resolved in v3.0.52 — IMAP service hardening.** The `catch` now logs at warn with the error and calls `clearCacheAll()` — both the observable signal and the conservative cache flush the audit asked for.
...
> **Resolved in v3.0.52 — IMAP service hardening.** `getEmails` now throws the new exported `IMAPNotConnectedError` on a connection/client failure instead of returning `[]`; the central tool dispatcher already serialises thrown errors. The legitimate empty-folder path (connection up, `total === 0`) still returns `[]`.
...
> **Resolved in v3.0.52 — IMAP service hardening.** New `isProtectedFolder()` helper (used by `deleteFolder` and `renameFolder`) trims+casefolds the input against the system-name set and, when folder discovery succeeds, also matches the server-reported `specialUse` against `['\\Inbox','\\Sent','\\Drafts','\\Trash','\\Junk','\\Archive','\\All','\\Flagged']`.
...
> **Resolved in v3.0.52 — IMAP service hardening.** The delete fallback in `chunkedBatchOp` now flags `\Deleted` per UID (cheap `STORE`) and runs exactly one trailing EXPUNGE of the flagged set via a new optional `finalize` callback (invoked only when the per-UID fallback actually ran). Both bulk-delete call sites pass it; semantics unchanged, N−1 EXPUNGE round-trips eliminated.
...
> **Resolved in v3.0.47 — Batch 6.** `processDue()` re-reads `item.status` after `await sendEmail(...)` and skips the `"sent"`/`"failed"` flip when a concurrent `cancel()` has already written `"cancelled"`. New `"sending"` status flipped + persisted before the await makes the cooperative-stop signal observable.
...
> **Resolved in v3.0.47 — Batch 6.** `cancel()` now returns a discriminated `CancelResult` and emits `{ ok: false, error: "in_flight" }` when the target is `"sending"`. The `cancel_scheduled_email` tool handler maps this to a user-facing warning that the message may still be delivered, instead of the prior misleading success.
...
> **Resolved in v3.0.50 — Batch 5.** New `sanitizeDcrClientName()` strips ANSI + control chars + length-caps at 100 chars at registration. Consent page now renders an "⚠ Untrusted client" badge for every `token_endpoint_auth_method === "none"` client. Page-level `esc()` tightened to 5-replacement (added `>` and `'`).
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The token-endpoint resource mismatch now returns the same opaque `invalid_grant` as the code/client/redirect/PKCE failures, so it is no longer a confirmed-good-code oracle.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `OAuthStore` keys both the `tokens` map and the `tokensByClient` reverse index by `sha256(token)`; the user-supplied bearer is hashed before any Map lookup, so the comparison is over fixed-length digests rather than the attacker-controlled string.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `/health` now returns `{ "status": "ok" }` only — no `transport` flavour, no `oauth` flag.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** A `setSecurityHeaders(res)` helper applies `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, and `Cache-Control: no-store` at the top of the listener, so every transport response carries them.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Both the authorize GET and POST handlers reject a `state` that contains any character outside `[\x20-\x7E]`.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `handleAuthorizePost` now binds every issued code to a resource: a caller-supplied value must be a valid absolute http(s) URI (`normalizeResource`), and an absent/empty one defaults to the canonical `cfg.resource`, so the per-token resource check is never silently skipped.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Already closed by the DCR sanitiser shipped in #145: `sanitizeDcrClientName` strips ANSI escape sequences and `[\x00-\x1f\x7f]` control characters (and length-caps) at registration, so the `callerClientName` flowing into `runWithCaller` and downstream logs/notifications is already clean. (The chat-mention vector on the webhook render path is separately closed under UI-017.)
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `AgentGrantStore.createPending` now caps the not-yet-reviewed queue at `MAX_PENDING_GRANTS` (50), evicting the oldest pending grant(s) before admitting a new one, so a registration flood can't grow the settings-UI list without bound. (The DCR hook lacks the caller IP, so the cap is enforced on the pending queue rather than per-IP.)
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Already fixed in code since #126 — `revokeTokensForClient` increments `n` only inside `if (this.tokens.delete(token))`. v3.0.63 adds the invariant test (`oauth-store.test.ts` "XPORT-013 — revokeTokensForClient leaves the reverse index consistent").
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `extractBearer` now matches `/^Bearer ([^\s]+)\s*$/i` — exactly one SP between the scheme and the token, optional trailing whitespace.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The `oauthEnabled && !oauthAdminPassword` throw now runs before `notifications.subscribe(...)`, so the subscription handle is never created when the config is rejected.
...
> **Resolved in v3.0.50 — Batch 5 (minimum fix).** `EscalationRecord` now carries `requestedByClientId` + `requestedByClientName` (control-char-stripped, length-capped); dispatcher threads it through. The deeper per-agent widening (vs. global) remains a follow-up but the information asymmetry is closed — operators see who requested the escalation before approving.
...
> **Resolved in v3.0.50 — Batch 5.** New `TOOL_ALIASES` + `canonicalToolName()` in `src/config/schema.ts`; applied at the rate-bucket lookup, per-tool enabled flag, and destructive-confirm gate.
...
> **Resolved in v3.0.50 — Batch 5.** New `MOVE_TOOLS_WITH_DESTRUCTIVE_TARGET` + `DESTRUCTIVE_DESTINATIONS` in `src/config/schema.ts`; the dispatcher destructive gate checks `args.targetFolder`/`args.folder` against the destination set (case-insensitive) for the three movers.
...
> **Resolved in v3.0.57 — permission/grant correctness.** The dispatcher now wraps the escalation-handler dispatch and writes an `agentAudit` row (ok / blockedReason + duration) for OAuth callers, so escalation requests are attributable in `~/.mailpouch-agent-audit.jsonl`. The tools still bypass the grant/permission/destructive gates by design — they can request, never grant.
...
> **Resolved in v3.0.57 — permission/grant correctness.** `PermissionManager.check` now returns `{ allowed: false, reason: "…not a registered tool…" }` when the (canonicalized) tool resolves to no permission entry, instead of failing open.
...
> **Resolved in v3.0.57 — permission/grant correctness.** New `withFileLock` util (sibling `${target}.lock` dir, atomic `mkdir` acquire + stale-break, no new dependency) wraps the load→mutate→atomic-rename cycles in `escalation.ts` (request/approve/deny/status/pending) and `grant-store.ts` (create/approve/deny/expire/prune). The grant store additionally reload-merges grants another process created so a whole-file rewrite no longer drops them.
...
> **Resolved in v3.0.57 — permission/grant correctness.** The handler takes one `configSnapshot = loadConfig() ?? defaultConfig()` near the top; the agent-grant gate's `globalPreset` and the destructive-confirm gate's `requireDestructiveConfirm` both read that snapshot, closing the TOCTOU window. (The permission gate's 15 s `cachedConfig` is a separate intentional hot-reload path.)
...
> **Resolved in v3.0.57 — permission/grant correctness.** The HTTP transport now refuses the static bearer whenever OAuth is enabled (`!oauthHandlers` guard on the static-bearer branch); OAuth deployments must use per-client DCR tokens that are individually gated, audited, and revocable. The static bearer remains available only for non-OAuth deployments.
...
> **Resolved in v3.0.57 — permission/grant correctness.** The registration side was already closed in v3.0.50 #145 (`sanitizeDcrClientName` strips ANSI/control chars + caps length). This batch closes the render side: the settings-UI grant/agent-list `esc()` helpers were a weak 3-replacement (`& < "`) and could pass `>`/`'` from `clientName`; they now escape all five (`& < > " '`), matching the strict `escHtml`.
...
> **Resolved (partial) in v3.0.57 — permission/grant correctness.** The four email-ID-scoped mutators whose service genuinely honors the folder (`delete_email`, `mark_email_read`, `star_email`, `get_email_by_id`) are removed from `FOLDER_AGNOSTIC_TOOLS`, and `extractFolderArg` now reads `sourceFolder`/`source_folder`. A folder-restricted grant enforces the allowlist against the call's `sourceFolder`; a call omitting it fails closed. **Residual:** `get_thread` and `download_attachment` remain folder-agnostic *by design* because gating them here would be false enforcement — `get_thread` assembles related messages from INBOX+Sent regardless of the seed folder, and `download_attachment` passes no folder to the IMAP service (UID resolved without a folder constraint). Honestly scoping those two requires service-level folder constraints (cross-folder thread filtering; attachment UID resolution pinned to `sourceFolder`) — tracked as a PERM-011 follow-up for a later batch rather than shipping a gate check the service doesn't back.
...
> **Resolved in v3.0.57 — permission/grant correctness.** `GrantManager.check` short-circuits a `custom` grant preset to its explicit `toolOverrides` (default-deny for un-overridden tools) before reaching `intersectPresets`, since `buildPermissions("custom")` is all-enabled and carries no real restriction. `intersectPresets` therefore never sees `custom` as the grant side; `custom` as the *global* ceiling stays at the top rank, with the live per-tool custom config still enforced by `PermissionManager.check`.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `src/index.ts` now calls `agentGrants.flushCounters()` on a 5-minute `setInterval().unref()` and again inside `gracefulShutdown`, so `totalCalls`/`lastCallAt` survive a restart.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `rotateIfNeeded` now `renameSync`s the live file to a `.rotating` staging name first, gzips that copy out-of-line into `.1.gz`, then unlinks it. A concurrent `appendFileSync` lands in a fresh live file rather than the snapshot being truncated, closing the read→truncate loss window.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Already closed in code — `requestEscalation` wraps the entire load→check→save cycle in `withFileLock(getPendingFilePath(), …)` (the same lock that closes PERM-006), so two parallel callers serialize and the `MAX_PENDING` invariant holds. The inline comment already credits PERM-014.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The supervised preset now iterates an explicit `SUPERVISED_BULK_ACTION_TOOLS` allowlist instead of `tool.startsWith("bulk_")`, so a future non-`bulk_`-prefixed mass tool must be added deliberately rather than slipping through unthrottled.
...
> **Resolved in v3.0.59 — crypto/credential discipline.** `decrypt()` and `isValidEncrypted()` now require the auth tag to decode to a full 16-byte GCM tag; short/garbage/over-length tags are rejected before they reach `setAuthTag`. Real blobs (v1→v3) always carry a 16-byte tag, so the read-side migration is unaffected.
...
> **Resolved in v3.0.59 — crypto/credential discipline.** `saveConfig` (config file) and the machine-id fallback now `statSync` → `chmodSync(0o600)` when the mode has drifted past `0o077`, mirroring the logger's chmod-on-detect pattern, instead of trusting the umask-masked `writeFileSync` creation mode. (Pass-audit re-assert is tracked separately under CRED-009.)
...
> **Resolved in v3.0.59 — crypto/credential discipline.** When a valid-shaped encrypted blob fails `decrypt()`, `loadCredentialsFromKeychain` emits a `logger.error` and returns a distinct `{ storage: "decrypt-failed" }` sentinel (not null — which the caller couldn't distinguish from "no credentials" and would answer by reading the plaintext field itself, re-opening the hole). The startup caller in `index.ts` checks that sentinel and refuses to fall back to the coexisting plaintext `connection.password`/`smtpToken`, leaving the credential empty (send/receive fails closed until re-encrypted). Plaintext + failed-auth blob is treated as tamper, not a migration fallback.
...
> **Resolved in v3.0.59 — crypto/credential discipline.** `writeRegistry` now records each account's actual `saveAccountCredentials` result (`keychainSaved = hadSecrets && keychainOk`) and derives `credentialStorage` from that, instead of inferring keychain success from the scrubbed on-disk shape — so an empty-field normalization can no longer produce a false-clean "keychain" badge.
...
> **Resolved in v3.0.52 — IMAP service hardening (bounded-cap fix).** `downloadAttachment` now rejects with a clear error when `att.size > MAX_ATTACHMENT_DOWNLOAD_BYTES` (25 MB, matching Proton's own limit) *before* the full-source re-fetch + base64 encode. True streaming of the body part is a larger refactor left as a documented follow-up; the cap is the proportionate guard against OOM.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Setup form now uses `data-submit="noop"`; a delegated `submit` listener in the nonce'd script block calls `preventDefault()`. No inline handlers remain in the UI.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Main UI CSP is now `script-src 'nonce-${cspNonce}'` with no `'unsafe-inline'`. Regression test in `src/settings/server.http.test.ts`.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Every remaining raw `${data.*}` interpolation is now `escapeHtml`'d, `escapeHtml` also escapes `'`, and the page CSP is `script-src 'self'; style-src 'self' 'unsafe-inline'` (no inline scripts on the page). Regression test in `src/settings/server.http.test.ts`.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Added `onShutdownRequested` to `ServerSecurityOptions`; `index.ts` wires it to `gracefulShutdown` (which calls `_trayInstance.destroy()` before exit). The endpoint falls back to `process.exit(0)` only when no callback is registered. Regression test asserts the callback fires and `process.exit` is not called.
...
> **Resolved in v3.0.55 — config read-modify-write serialized by the CRED-008 file lock (#167).** Verified: `saveConfig` and `writeRegistry` in `src/config/loader.ts` now run their full read-modify-write under `withConfigLock`/`withConfigWriteLockAsync` — an `O_EXCL` `${config}.lock` with stale reclamation + reentrancy, plus an in-process async mutex — so the settings-server POST `/api/config` write and the MCP-side periodic writer can no longer clobber each other.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Reads now distinguish ENOENT (safe to start fresh) from any other read error (bail). A present-but-unparseable file returns `{ ok: false }` and is left untouched. Regression test in `src/settings/server.claude-desktop.mocked.test.ts`.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** `toolOverrides` is filtered to known `ALL_TOOLS` keys with boolean values; `conditions` is rebuilt key-by-key from the `GrantConditions` whitelist with per-value type checks. Unknown and `__proto__`-style keys are dropped. Regression test verifies no prototype pollution and that only sanitized values are stored.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Already closed in v3.0.57 (#168): the three local `esc` helpers in `shell.ts` (lines 377/578/620) are now full 5-replacement functions (`&`, `<`, `>`, `"`, `'`), identical to the module-level `escHtml` at line 2092. The 3-replacement form the finding describes no longer exists.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `defaultRunner` now arms a 3 s `setTimeout` that `child.kill("SIGKILL")`s the notifier subprocess and resolves `{ code: -1 }`; the timer is cleared on a normal close/error and is `.unref()`'d.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Both sites now resolve the Claude Desktop config path through a shared `claudeDesktopConfigPath()` helper that returns `null` on Windows when `%APPDATA%` is unset; the status route reports `found:false, configPath:null` and the write route refuses with an actionable error instead of building a CWD-relative path.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The agent `approve`/`deny`/`revoke` routes and the account `activate` route now each call `escalationLimiter.check(`${ip}:…`)` (20/min) before the CSRF check, matching the escalation routes.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Both halves are already satisfied: the local `esc` used for `condsJson` is now the full 5-replacement helper (UI-004, v3.0.57), and the server-side `/api/agents/:id/approve` route validates `conditions` against a key whitelist with per-value type checks (UI-011, v3.0.60). The client never round-trips unvalidated condition data.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The GET `/` handler now calls a new `hasValidBootstrapToken(req, url, accessToken)` in LAN mode. Because a top-level browser navigation cannot send the `X-Access-Token` header, the bootstrap check accepts the token from EITHER the header (SPA re-fetch) OR the documented `?token=` query (constant-time compared); API endpoints stay strictly header-only. A LAN device that GETs `/` without the token now receives 401 instead of the CSRF-token-bearing HTML.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** Every failed-bind path (EADDRINUSE-exhausted and generic) now calls a new `_markSettingsUnavailable()` that sets `_settingsEnabled = false`, clears `_settingsUrl`, and rebuilds the tray — so the `_settingsEnabled === !!_settingsUrl` invariant holds and "Open Settings" can't appear with an empty URL.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `buildPayload` runs `clientName` through `neutralizeChatMentions` for the slack/discord formats — `@` is split with a zero-width joiner (breaks `@here`/`@everyone`/`<@id>`), and `<`/`>`/backtick are replaced so a `<url|label>` link can't render. The raw/CloudEvents payloads carry the name as inert JSON data.
...
> **Resolved in v3.0.51 — Batch 7.** Replaced `defaultFetchMock()` with a per-test `seedUids(client, folder, uids)` helper backed by a `MockClientState` table. Default is empty (no UIDs in any folder); the `fetch` mock now only yields UIDs seeded into the currently-locked folder. 40 legacy tests updated with explicit seeding.
...
> **Resolved in v3.0.51 — Batch 7.** The mock client now tracks `lockedFolder` (set by `getMailboxLock`, cleared by `lock.release()`); `messageMove` / `messageDelete` / `messageFlagsAdd` / `messageFlagsRemove` / `messageCopy` reject when the requested UID isn't seeded into the locked folder. Combined with TEST-001's per-folder seed table this means a test that locks INBOX and moves a UID seeded only in Sent now fails loudly.
...
> **Resolved in v3.0.51 — Batch 7.** Both tests now pull the raw MIME Buffer from `appendMock.mock.calls[0][1]` and assert that the injected text never appears as its own MIME header line (`\r?\n\s*X-Injected:` / `\r?\n\s*X-Evil:`), the smuggled `\x01` control char is gone, and the visible filename / Message-ID tokens survive — pinning behaviour, not just absence.
...
> **Resolved in v3.0.51 — Batch 7.** Wrapped the test in `vi.useFakeTimers()` + `vi.setSystemTime(new Date("2026-03-15T00:00:00Z"))` (cleanup in `finally` block); the prune now consistently sees the old fired record as outside the 30-day window regardless of the host's wall clock.
...
> **Resolved in v3.0.51 — Batch 7.** Added a parallel `describe('Keychain (positive-path with stub @napi-rs/keyring) — TEST-005', ...)` block. The keychain.ts module uses `new Function("specifier", "return import(specifier)")` to keep `@napi-rs/keyring` optional, which is opaque to `vi.mock`; a new exported `__setKeyringForTests(stub | null)` test-only hook (paired with the existing `__resetKeyringCacheForTests`) lets the suite inject a fresh stub Entry backend in `beforeEach` so previously-set "entries" don't leak. Six new tests cover: `isKeychainAvailable=true` when backend resolves, `save→load` round-trip with call-arg assertions on the stub, aux credential round-trip (CRED-001), full six-field `migrateFromConfig` migration (password / smtpToken / remoteBearerToken / remoteOauthAdminPassword / passAccessToken / simpleloginApiKey blanked on disk + `credentialStorage` promoted to `keychain` + `saveFn` invoked exactly once), overwrite-on-collision, and `deleteCredentials` clearing both the stub state and the in-memory entries.
...
> **Resolved in v3.0.51 — Batch 7 (SMTP gap closed; alias/pass remain skip-stubbed).** Added `test/e2e/scenarios/sending.e2e.test.ts` with four Greenmail-backed scenarios for `send_email` (simple text body cross-user delivery, To/Cc/Bcc header propagation incl. Bcc absence-from-wire, HTML body with text/html assertion) and `send_test_email`. Each opens a second IMAP connection as Bob to verify the message actually arrived — the property that catches false-success counters. Two minimal opt-in production changes to make this work against Greenmail: `smtp-service.ts` drops `requireTLS` when `allowInsecureBridge` is set (mirrors IMAP TLS-pinning opt-in; default production behaviour unchanged), and a new optional `MAILPOUCH_SMTP_FROM` env var supplies a domain-qualified From so nodemailer doesn't build `MAIL FROM:<>` from the bare Greenmail login (production never sets this env var). Skip-with-reason placeholders added for `alias_*` (`MAILPOUCH_E2E_SIMPLELOGIN=1` gate) and `pass_*` (`MAILPOUCH_E2E_PASS=1` gate) so the audit gap stays visible. The remaining tool coverage (`schedule_email`, `cancel_scheduled_email` positive, `remind_if_no_reply`, `check_reminders`, `download_attachment`, `extract_action_items`, etc.) is out of scope for Batch 7 — tracked for a follow-on batch.
...
> **Resolved in v3.0.49 — Batch 9.** `prepare` is now build-only; a new `postinstall` delegates to `scripts/install-hooks.mjs`, which installs `simple-git-hooks` only when `INIT_CWD === process.cwd()` AND `.git/` exists at the package root. Downstream consumer installs (`pnpm add mailpouch`, `npm i github:chandshy/mailpouch`) no longer write git hooks into the consumer's repo.
...
> **Resolved in v3.0.49 — Batch 9.** Both jobs in `.github/workflows/publish.yml` now run `npm ci --ignore-scripts`, followed by an explicit `npm rebuild better-sqlite3 @napi-rs/keyring` — the only natives mailpouch's prod surface needs. Transitive `postinstall` shells no longer execute with `NPM_TOKEN` / OIDC publish keys in scope. `NPM_TOKEN` rotation is out of scope for this PR (environment-level concern).
...
> **Resolved in v3.0.49 — Batch 9.** All 13 `uses:` lines across `ci.yml`, `preship.yml`, `publish.yml`, and `build-native-tray.yml` are pinned to 40-character SHAs with `# vX.Y.Z` trailing comments. Versions standardized: `actions/checkout@de0fac2e...` (v6.0.2), `actions/setup-node@48b55a01...` (v6.4.0), `actions/upload-artifact@043fb46d...` (v7.0.1), `dtolnay/rust-toolchain@29eef336...` (branch HEAD of `stable` as of 2026-05-29 — the action does not cut tags, so the comment annotates the date instead). `preship.yml`'s `@v4` references were bumped to the same v6/v7 SHAs as the other workflows. SHA pins prevent silent action-tag-tampering: a compromised action repo can no longer reroute `actions/checkout@v6` to malicious code on our next workflow run.
...
> **Resolved in v3.0.43 — preship gate hotfix (#130).** Verified: `check-licenses.mjs` partitions `flat` into `usable` (kept) and `skipped` (`version === "(unknown)"`, excluded from the baseline + drift comparison and surfaced as a separate diagnostic line); the baseline diff also filters `(unknown)` entries on the read side.
...
> **Resolved in v3.0.43 — preship gate hotfix (#130).** Verified: the `npm-version-free` step in `scripts/preship.mjs` now treats exit 0 + version output as "ALREADY PUBLISHED" (`ok:false`), an E404/"not found" as "not yet published" (`ok:true`), and any other non-zero (registry unreachable) as "could not verify" (`ok:false`) — no longer false-greens on an unreachable registry.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Added an `other` bucket for severities outside high/critical/moderate/low (e.g. `info`/`unknown`); these are now printed and exit 2 (advisory) rather than silently dropped.
...
> **Resolved in v3.0.43 — preship gate hotfix (#130).** Verified: when `LICENSES.json` is absent, `check-licenses.mjs` errors actionably ("no baseline … generate one with `PRESHIP_LICENSE_WRITE=1`") and exits 1 without writing the file; the baseline is only written under the explicit `PRESHIP_LICENSE_WRITE=1` flag.
...
> **Resolved in v3.0.43 — preship gate hotfix (#130).** Verified: after the `--version` boot, `smoke-tarball.mjs` runs `tar -tzf <tarball>` and asserts the packed set contains `package/native/tray/index.js`, `…/index.d.ts`, `package/dist/index.js`, `…/settings-main.js`, and `…/utils/tray.js` — so a dropped `files` entry now fails the gate.
...
> **Resolved (partial) in v3.0.60 — UI/CSP + build/docs sweep.** Enabled the two zero-error flags now: `noImplicitOverride` and `isolatedModules`. `noUncheckedIndexedAccess` and `exactOptionalPropertyTypes` cascade many errors and are deferred to a dedicated follow-up exactly as the finding recommends ("turn on in a follow-up PR; fix incrementally").
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Took the glob-exclude route: `files` now contains `!dist/**/*.js.map` and `!dist/**/*.d.ts.map`. Verified via `npm pack --dry-run` that 0 `.map` files are packed.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** Deferred-after-halt steps now render with a red `↷` mark and the literal text `deferred after halt` (table row + summary), instead of gray "skipped" — a stdout parser can no longer conflate them with an intentional skip/pass.

> **Resolved in v3.0.43 — preship gate hotfix (#130).** Verified: `scripts/preship.mjs` honours `PRESHIP_SKIP=1` at the top of the file (short-circuits with a loud `BYPASS: PRESHIP_SKIP=1` line to stderr and exits 0), and `docs/preship.md:138-139` documents the bypass for both `/ship` and `npm run preship*`.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** `tarball-smoke` moved from `heavySteps` into `fastSteps`, so it now runs on every pre-push (`preship:fast`).
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** All early-exit branches in `smoke-tarball.mjs` now `throw new Error(...)` inside the `try`, so the existing `catch` sets `exitCode = 1` and the `finally` always runs the staging/install cleanup; no bare `process.exit(1)` remains in the body.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The `fast-xml-parser` override is removed from `package.json` (`package-lock.json` has zero references to it, confirming it never matched a real dep).
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `spawnNpmRun` now runs `npm run <script>` without `--silent`, so npm's `npm error` framing surfaces on a failing script (the runner already captures and prints child stdout+stderr on failure).
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `spawnStep` now scrubs the publish-only secrets `NPM_TOKEN`/`NODE_AUTH_TOKEN`/`GITHUB_TOKEN`/`GH_TOKEN` from every child env by default (a step can opt one back in via `keepEnv`). The Bridge config env var stays available to the e2e:bridge step.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `test:e2e:local` now runs `node scripts/wait-for-port.mjs 127.0.0.1 3143 30` between `docker compose up -d` and the vitest run, so vitest only starts once Greenmail's IMAP port accepts a connection.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** A repo-local `.npmrc` with `engine-strict=true` now turns the `engines.npm: ">=9.0.0"` constraint into a hard error for anyone installing from the repo. (`.npmrc` is not in the `files` allowlist, so it doesn't ship to consumers.)
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `check-secrets.mjs` excludes `test/**/fixtures/**` and `**/__fixtures__/**` from the grep-fallback pathspecs. Real `*.test.ts` files stay scanned (so an accidental real leak in test code is still caught); only the deliberate fixtures trees are exempt.
...
> **Acknowledged in v3.0.63 — by design: npm writes resolved `overrides` versions into `package-lock.json`, so a meaningful override change updates the lock and invalidates `setup-node`'s `cache: 'npm'` key in practice. Adding a parallel `actions/cache` step keyed on both files would duplicate caching work for a theoretical edge (an override that resolves to an already-pinned version). Not worth the complexity at this confidence level (Suspect).**
...
> **Acknowledged in v3.0.63 — by design: no ESLint config exists in this repo, and the `lint`/`lint:fix` aliases of `tsc --noEmit` plus the hardcoded `{ ok: true, summary: "alias of typecheck" }` preship slot keep the step table stable across fast/full/release until a real linter is wired (the slot is explicitly commented as a placeholder). Deleting the slot would churn the table for no behavioural gain; wiring ESLint is a deliberate follow-up, not a low-severity cleanup.**
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The GPR publish step now `cp package.json /tmp/pkg.bak` before the scoped-name rewrite and `mv /tmp/pkg.bak package.json` after `npm publish`, so any later step in the job sees the original workspace.
...
> **Acknowledged in v3.0.63 — by design: `mailpouch-settings` is the *documented, user-initiated* launcher for the settings UI (README and HELP both teach `npx mailpouch-settings [--lan|--port|--browser|--tui]`). The settings-UI lifecycle policy targets *persistent/autostart* launches; a user explicitly running `npx mailpouch-settings` is on-demand and agent-external by definition, not a daemon. Removing the bin would break every documented invocation with no replacement. Keeping it.**
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The `/tmp/preship-pack-*` and `/tmp/preship-smoke-*` lines are removed; `smoke-tarball` stages into `os.tmpdir()` (never the repo), and the `mailpouch-*.tgz` line covers a stray local `npm pack`.
...
> **Resolved in v3.0.43 — preship gate hotfix (#130).** Verified: `scripts/preship.mjs` registers `npm-audit` as `mode: "hard"` and maps the check script's exit 1 (HIGH/CRITICAL) to `ok:false`; `docs/preship.md:30` documents "HIGH/CRITICAL block, MODERATE/LOW are advisory". Doc and code now agree.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** README:319 now reads "capped at 20/hr in `supervised`", matching `loader.ts` and README:352.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** README:328 now reads "capped at 5/hr in `supervised`", matching `loader.ts:125-126`.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** HELP.md:163 example now uses `sinceEpoch: 1735689600`, matching the `fts_search` schema.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** HELP.md:194 example now uses `email_id` / `after_days`, matching the `remind_if_no_reply` schema.
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** README_FIRST_AI.md:680-688 now documents inputs `target_preset` + `reason` (both required), drops the bogus `newTools` input row, and corrects the return shape to `{ status: "pending", challenge_id, … }`. (`newTools` exists only in the *output* structuredContent, not as an input.)
...
> **Resolved in v3.0.60 — UI/CSP + build/docs sweep.** HELP.md:275 now points to `~/.mailpouch.audit.jsonl`; the file list relabels it "escalation audit log" and drops the non-existent `~/.mailpouch-escalation-audit.jsonl`. (Confirmed `escalation.ts` is the sole writer of that path.)
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** HELP.md:107 now lists `delete_folder` alongside the other `DESTRUCTIVE_TOOLS` entries.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** README System category row now reads `5`, matching `TOOL_CATEGORIES.system.tools`.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** The README env-var table now documents `MAILPOUCH_REMINDERS`, `MAILPOUCH_PASS_AUDIT`, `MAILPOUCH_FTS_DB`, `MAILPOUCH_AGENTS`, `MAILPOUCH_AGENT_AUDIT`, and `MAILPOUCH_MACHINE_SECRET` with their code defaults.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `docs/index.md` "System / Bridge" row now includes `get_server_version`.
...
> **Resolved in v3.0.63 — transport/permission/UI/build/docs low-severity sweep.** `schema.ts:372` and README now both read "27 tools + 2 escalation = 29 visible", matching the authoritative comment at `schema.ts:477`.
