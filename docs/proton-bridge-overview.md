...
mailpouch is tested and pinned against **Bridge v3.25.x (Oberbaum)** as the current supported baseline (latest stable v3.25.0, released 2026-06-01). The IMAP/SMTP protocol surface has been stable across the v3.20–v3.25 series, so older v3.20+ installs continue to work, but these Bridge-side behaviour changes are worth noting:

- **Security patches (v3.25.0, 2026-06-01)** — Bridge ships "patch vulnerabilities for March 2026" (BRIDGE-483) and sanitises attachment filenames in user debug mode (BRIDGE-512). Proton flags this release as strongly recommended. Note that Bridge self-updates independently of the OS package manager, so `dpkg`/`rpm` version output can lag badly behind the running binary — always confirm with `protonmail-bridge --version`.
- **MIME boundary generation is now stochastic (v3.25.0, BRIDGE-548)** — Bridge randomises MIME boundaries when building messages. Nothing in mailpouch keys off boundary strings, but any future logic that fingerprints or dedupes on raw MIME structure must not assume boundaries are stable across builds of the same message.
...
- **Label endpoint mapping (v3.23.0 → v3.24.1, March–April 2026)** — Bridge changed how it translates IMAP label-folder operations (`UID COPY` into `Labels/<name>`, `UID STORE \Deleted` against a label folder) into upstream Proton API calls. v3.23.0 stopped calling the unlabel endpoint when moving between folders; BRIDGE-485 (v3.24.0) separated expunge-from-old-location from the unlabelling logic; BRIDGE-488 (v3.24.0) reverted `RemoveFromMailbox` to its pre-3.23.0 behaviour; BRIDGE-522 (v3.24.1) restored the old unlabel endpoint behind a feature flag.

  **Why this matters:** `remove_label` / `bulk_remove_label` are implemented as `\Deleted` + `EXPUNGE` against the `Labels/<name>` mailbox. That is only non-destructive because Bridge maps an expunge from a label mailbox to "remove this label", not "delete this message". If that mapping ever flips, unlabelling becomes a permanent delete.

  **Verified empirically on Bridge v3.25.0 (2026-07-30):** labelled a message, unlabelled it, and confirmed it was detached from `Labels/<name>` while surviving in its source folder. The invariant holds today. It is now guarded by the `remove_label detaches the label and leaves the message alive in its source folder` case in `test/e2e/scenarios/safe-bridge.e2e.test.ts` — **bridge-lane only**, because a Greenmail `Labels/` mailbox is an ordinary folder where a genuinely destructive expunge would pass an "it left the label" assertion just as happily. Re-run the safe-bridge lane when upgrading Bridge.
